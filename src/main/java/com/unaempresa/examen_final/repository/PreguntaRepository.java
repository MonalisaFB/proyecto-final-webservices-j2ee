...
public interface PreguntaRepository extends JpaRepository<Pregunta, Long> {
    ...
    @Query("SELECT p FROM Pregunta p WHERE (:tematicaId IS NULL OR p.tematica.id = :tematicaId) AND (:tipo IS NULL OR TYPE(p) = :tipo)")
    Page<Pregunta> filtrar(@Param("tematicaId") Long tematicaId, @Param("tipo") Class<? extends Pregunta> tipo, Pageable pageable);
}